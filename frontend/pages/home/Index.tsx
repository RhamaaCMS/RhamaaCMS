...
export default function Home({ site_name }: HomeProps) {
  return (
    <RootLayout>
      ...
      <section className="relative flex min-h-[calc(100vh-4rem)] flex-col overflow-hidden bg-black">
        {/* Background grid */}
        <div
          className="pointer-events-none absolute inset-0 opacity-[0.035]"
          style={{
            backgroundImage:
              "repeating-linear-gradient(45deg,rgba(255,255,255,0.16) 0,rgba(255,255,255,0.16) 1px,transparent 1px,transparent 28px),repeating-linear-gradient(-45deg,rgba(255,255,255,0.12) 0,rgba(255,255,255,0.12) 1px,transparent 1px,transparent 28px)",
          }}
          aria-hidden="true"
        />

        {/* Ambient glow */}
        <div className="pointer-events-none absolute -right-32 -top-32 h-[480px] w-[480px] rounded-full bg-zinc-800 opacity-35 blur-[96px]" aria-hidden="true" />
        ...
        <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-4 py-20 text-center">
          ...
          {/* Tagline */}
          <p className="animate-fade-up animation-delay-400 mb-8 mt-1 max-w-sm text-sm leading-relaxed text-zinc-400">
          ...
          <div className="animate-fade-up animation-delay-500 mb-8 flex w-48 items-center gap-3">
            ...
            <svg className="h-2.5 w-2.5 shrink-0 text-zinc-500/60" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
          ...
          <div className="animate-fade-up animation-delay-600 grid w-full max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3">
            {FEATURE_CARDS.map((card) => (
              <a
                ...
                className="group flex flex-col gap-3 rounded-2xl border border-white/12 bg-white/[0.04] p-5 text-left transition-all duration-200 hover:-translate-y-1 hover:border-white/35 hover:bg-white/[0.09]"
              >
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/15 bg-white/[0.06] text-zinc-100">
                  {card.icon}
                </div>
                ...
                <span className="flex translate-x-[-4px] items-center gap-1 text-xs font-medium text-zinc-200 opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100">
                  {card.cta}
                </span>
              </a>
            ))}
          ...
          <div className="animate-fade-up animation-delay-700 mt-7">
            <a
              ...
              className="inline-flex items-center gap-1.5 font-mono text-[11px] text-zinc-500 transition-colors duration-200 hover:text-zinc-200"
            >
